...
const baseURL = 'https://main--jmp-dev--jmphlx.hlx.live';
...
async function sendDeleteRequest(authToken, page, deindex) {
  //'https://admin.hlx.page/index/jmphlx/jmp-dev/main/en/online-statistics-course/request-access-to-teaching-materials/download-teaching-materials' 

  console.log(authToken);
  console.log(page);
  let url;
  if (deindex) {
    url = `https://admin.hlx.page/index/jmphlx/jmp-dev/main${page}`;
  } else {
    url = `https://admin.hlx.page/live/jmphlx/jmp-dev/main${page}`;
  }
...
function buildEmailBody(successPages, failedPages, region) {
  ...
  if (failedWorkflow) {
    ...
    failedPages.forEach((page) => {
      emailBody += `<li><a href="https://da.live/edit#/jmphlx/jmp-dev${page}">${page}</a></li>`;
    });
    emailBody += '</ul></div>';
